...
function create_intent($mon_id, $requestor_id, $processor_id, $type_reference, $requestor_notes = "", $processor_notes = "")
{
...
LEFT JOIN
    ...
    $stmt = $db->prepare($query);
    try {
        $stmt->execute([":tid" => $type_reference, ":pokemon_id" => $mon_id]);
        $result = $stmt->fetch();
        if ($result) {
    ...
    try {
        $r = $stmt->execute([
            ":pokemon_id" => $mon_id, ":requestor_id" => $requestor_id, ":processor_id" => $processor_id, ":type" => $intent_type_id,
...
JOIN CA_Pokemon as c on c.id = ci.pokemon_id LEFT JOIN Users as pu on pu.id = processor_id
WHERE 1=1";
    $query = "SELECT ci.id, ci.pokemon_id, requestor_id, c.name as pokemon_name, pu.username as processor_name, cis.label as intent_status, cit.label as intent_type, requestor_notes, processor_notes, ci.modified
